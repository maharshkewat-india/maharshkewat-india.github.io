...
function DetailCard({ title, children, id }: DetailCardProps) {
  ...
  return (
    <section className="relative overflow-hidden rounded-xl border border-white/10 bg-[#0b1018]/70 p-5 shadow-lg shadow-black/10 backdrop-blur-sm sm:p-6" aria-labelledby={headingId}>
      <span aria-hidden="true" className="absolute inset-x-6 top-0 h-px bg-gradient-to-r from-transparent via-cyan-300/40 to-transparent opacity-70" />
...
export default function LabCaseStudy({ lab }: { lab: CybersecurityLab }) {
  return (
    <article id={lab.slug} className="scroll-mt-24 border-t border-white/[0.08] pt-12 first:border-t-0 first:pt-0">
      <div className="flex flex-col gap-4 border-b border-white/[0.08] pb-8 sm:flex-row sm:items-start sm:justify-between">
        ...
        {lab.eventId && <span className="shrink-0 self-start rounded border border-cyan-300/25 bg-cyan-300/[0.04] px-3 py-1.5 font-mono text-xs uppercase tracking-[0.1em] text-cyan-200">Event {lab.eventId}</span>}
      </div>
      ...
      <div className="mt-4 grid gap-4">
        ...
        <DetailCard id={lab.slug} title="Evidence / Screenshots">
          <div className="flex min-h-40 items-center justify-center rounded-lg border border-dashed border-cyan-300/25 bg-black/20 px-6 text-center text-sm leading-6 text-slate-400">
      ...
      <div className="mt-8 flex">
        <a href={lab.github} target="_blank" rel="noopener noreferrer" className="inline-flex items-center rounded-md bg-cyan-300 px-5 py-3 text-sm font-bold text-[#061018] transition hover:bg-cyan-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-100 focus-visible:ring-offset-2 focus-visible:ring-offset-[#0B1018]">
